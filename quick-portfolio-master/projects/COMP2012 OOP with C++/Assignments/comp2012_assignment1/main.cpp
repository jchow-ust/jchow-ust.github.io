...
_test_table()
{
	...
	cout << endl;
	d->print();
	d->deleteAttribute(2); cout << endl;
	d->print();
	d->deleteAttribute(0); cout << endl;
	d->print();
...
_test_intermediate()
{
	...
	// JZ debug
	cout << "jz was here" << endl;

	Table newcourses;
	newcourses.addAttribute("Department");
	newcourses.addAttribute("Course Code");
	newcourses.addAttribute("Course Name");
	newcourses.addAttribute("Credits");
	newcourses.addEntry(comp2012);
	newcourses.addEntry(comp3021);
	newcourses.addEntry(comp3711);
	newcourses.addEntry(comp4511);
	newcourses.addEntry(comp4641);
	newcourses.addEntry(math2343);
	newcourses.addEntry(math3033);
	newcourses.addEntry(math4023);
	//cout << courses.numEntries << endl;

	newcourses.query().select();

	//cout << newcourses.addAttribute("Semester", 1, "2020S") << endl;
	//bool result = newcourses.addAttribute("Semester"); cout << result << endl;
	//bool result = newcourses.addAttribute("Semester", -2, "2020S"); cout << result << endl;
	//bool result = newcourses.addAttribute("Semester", 6, "2020S"); cout << result << endl;
	//string newstring[] = {"CSE", "2021F", "COMP3511", "Operating Systems", "3"};
	//cout << newcourses.addEntry(newstring, 2) << endl;
	//newcourses.query().select();
	//newcourses.deleteAttribute(1);
	//newcourses.deleteEntry(2);
	//newcourses.query().select();

	//newcourses.query().where("Department", EQ, "CSE").select();
	//newcourses.query().where("Semester", EQ, "2020S").select();
	//newcourses.query().orderBy("Department", DESCENDING).select();
	//newcourses.query().limit(2).select();

	//newcourses.query().select(selector, 2);
	//newcourses.query().where("Course Name", CONTAINS, "Programming").update("Credits", "5");
	//newcourses.query().select();
	//newcourses.query().where("Department", EQ, "CSE").orderBy("Course Name", ASCENDING).orderBy("Credits", DESCENDING).limit(3).select();
