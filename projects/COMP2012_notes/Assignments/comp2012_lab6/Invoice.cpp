...
void Invoice::createTransaction(int _qty){
	if(_qty <= productSale->getUnit()){
		...
		if (dynamic_cast<Customer*>(buyer)==nullptr){return;}
		float dis = dynamic_cast<Customer *>(buyer)->getDiscount();
		if (dis==-1){
			cout << "Error in discount value, transaction cancelled!" << endl;
			return;
		}
		else{
			cost = _qty*productSale->getUnitPrice()*dis;
			productSale->setUnit(productSale->getUnit()-_qty);
...
void Invoice::print() const{
	cout << "Invoice: " << id << " | Date: " << date << " Quantity: " << quantity << " | Cost: " << cost << " | Buyer: " << buyer->getName() << " | Salesman: " << seller->getName() << " | Product: " << productSale->getName();
	cout << endl;
}
