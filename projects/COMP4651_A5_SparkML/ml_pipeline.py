...
plantDF = sqlContext.read.format('com.databricks.spark.csv').options(
  delimiter='\t', header='true',inferschema='true').load("/databricks-datasets/power-plant/data")
...
# all of them should be DoubleType() and nullable
...
altPlantDF = sqlContext.read.format('com.databricks.spark.csv').options(
  delimiter='\t', header='true').load("/databricks-datasets/power-plant/data", schema=customSchema)
...
(split15DF, split85DF) = datasetDF.randomSplit([0.15, 0.85], seed=1900009193)
...
# set evaluator to be regression, and use 3 folds
