...
# We notice that in formatting, the rest of the RE is unchanged, the only parts that the current re has difficulty parsing is within the ""
# ie the "<requestMethod> <endpoint> <clientProtocolVersion>" request.
# \s matches all characters considered whitespace
# \S matches any character which is not whitespace (the opposite of \s)
# before: (\S+) (\S+)\s*(\S*) = first (\S+) gets requestMethod, 2nd (\S+) gets endpoint, then gets 0 or more whitespaces, then gets clientProtocolVersion
# problem: there is a 4th additional argument in some records between <endpoint> and <clientProtocolVersion>
# solution:
# "(\S+)": <requestMethod>
# "(\S+)\s*": <requestMethod> + whitespaces between it and <endpoint>
# "(\S+)\s*(\S+)\s*": <requestMethod> + whitespaces between it and <endpoint> + <endpoint> + whitespaces between <endpoint> and either <cPV> or 4th arg
# "(\S+)\s*(\S+)\s*[\S*\s*]* ": <requestMethod> + whitespaces between it and <endpoint> + <endpoint> + whitespaces between <endpoint> and either <cPV> or 4th arg + Kleene star (since we may not have 4th arg) of a (set of space separated chars) + " " to separate 4th arg from <cPV>
# "(\S+)\s*(\S+)\s*[\S*\s*]* (\S*)": capture <cPV> as well
APACHE_ACCESS_LOG_PATTERN = '^(\S+) (\S+) (\S+) \[([\w:/]+\s[+\-]\d{4})\] "(\S+)\s*(\S+)\s*[\S*\s*]*\s*(\S*)" (\d{3}) (\S+)'
...
topFiveErrURLs = endpointSum.takeOrdered(5, lambda x: -x[1]) # sort in ascending order of -ve, so highest counts first
...
# (hour, host) -> remove potential duplicates -> group by key aka 'hour'
hourGroupedHosts = (hourToHostPairTuple
                    .distinct()
                    .groupByKey())

# then make it into (hour, [host1, host2, ...]) and sort by the key 'hour'
hourHostCount = (hourGroupedHosts
                 .map(lambda x: list(x))
                 .sortByKey())

hourlyHosts = (hourHostCount
...
# join by keys 'hour'. Then we have something like (0, (9, 2)) 
# where 0 is the hour, 9 is the total number of requests for that hour (from sortedByHour), 2 is the number of unique hosts (from hourlyHosts)
avgHourlyReqPerHost = (sortedByHour.join(hourlyHosts).mapValues(lambda x: x[0]//x[1])).sortByKey().cache()
...
badEndpointsTop10 = badEndpointsSum.top(10, lambda x: x[1])
