...
# TODO: Replace <FILL IN> with appropriate code
capitalLambdaRDD = wordsRDD.map(lambda w: w.upper())
...
# HCJ: this will group k-v pairs by their key (a word) and a list of ints whose sum is the number of times that word has appeared. The lambda has to return an iterable
...
# TODO: Replace <FILL IN> with appropriate code
# converts (dog, 1), (dog, 2), (cat, 2) to (dog, 3), (cat, 2)
...
# TODO: Replace <FILL IN> with appropriate code
# map converts each word to (word, 1), then reducebykey groups by words and takes ('dog', 12), ('dog', 1) = ('dog', 13)
...
totalCount = (wordCounts
              .map(lambda w: w[1]) # maps each k-v to its value (since we don't care about the key, just want the sum of ALL v's)
              .reduce(add))
...
def removePunctuation(text):
    ...
    """
    # the ^ means replace any Kleene plus of (char not in {A-Z,a-z,0-9,space}) with '' (empty str),
    # and then change to lower case then remove leading (lstrip) and trailing (rstrip) spaces
...
# TODO: Replace <FILL IN> with appropriate code
shakespeareWordsRDD = shakespeareRDD.flatMap(lambda line: line.split()) # split each line by spaces into its constituent words
...
# TODO: Replace <FILL IN> with appropriate code
top10WordsAndCounts = wordCount(shakespeareWordsRDD).takeOrdered(10, lambda w: -w[1]) # use -v to count 10 most common words. use v to count 10 least common words. this is bc the 2nd arge (the lambda) is the function we sort by, and sort by -ve means we sort in ascending order (starting from the most negative aka largest magnitude aka largest occurrence) 
