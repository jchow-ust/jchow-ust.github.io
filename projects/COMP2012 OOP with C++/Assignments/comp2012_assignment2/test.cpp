...
void runTestCase1() {
    ...
    delete farmland;
    // jz add
    //delete employee1; delete employee2; delete employee3;
...
void runTestCase2() {
    ...
    delete []employee_list;
    delete cattlefarm;
    //delete feeder; delete cow; delete cow1;
...
void runTestCase3()
{
    ...
    Scene* s = new Scene();
    int tmp3_init_money = s->getMoney();  // 200 to start
    ...
    delete s;
    // jz Step 5: Test removeProperty
    s = new Scene();
    s->addProperty(CATTLEFARM,6,4);
    s->addProperty(FARMLAND,0,10);
    Object * prop1 = s->getObjectAt(6,4);
    Object * prop2 = s->getObjectAt(0,10);
    s->hire(dynamic_cast<Property *>(prop2), FARMER);
    s->hire(dynamic_cast<Property *>(prop2), EMPLOYEE);

    s->hire(dynamic_cast<Property *>(prop1), COW);
    s->removeProperty(dynamic_cast<Property *>(prop1));

    int tmp3_5a;
    const Object** object_list_2;
    s->getConstObjects(object_list_2, tmp3_5a);
    if (tmp3_5a == 3 && dynamic_cast<Property*>(prop2)->getNumEmployee()==2)
    	cout <<"->5a/"<<steps<<" PASS."<<endl;
    else {
    	cout <<"->5a/"<<steps<<" FAILED."<<endl;
    	exit(-1);
    }
    s->removeProperty(dynamic_cast<Property *>(prop2));
    int tmp3_5b;
    const Object** object_list_3;
    s->getConstObjects(object_list_3, tmp3_5b);
    if (object_list_3 == nullptr && tmp3_5b == 0)
    	cout <<"->5b/"<<steps<<" PASS."<<endl;
    else {
    	cout <<"->5b/"<<steps<<" FAILED."<<endl;
    	exit(-1);
    }
    // end jz Step 5
    delete s;
    delete []object_list_2; // obj_list_1 and 3 didnt have to be deleted since it was nullptr in the passing test case
    cout << "->TEST CASE 3: PASS." << endl << endl;
    return;
}

void runSegFaultTest(){
	cout << "Test Case segfault" << endl;
	...
	s->hire(dynamic_cast<Property*>(selected_object), COW);
	//s->hire(dynamic_cast<Property*>(selected_object), FEEDER);
	// money should equal 50 here
	// make money 6 times to 110, then on 7th day Feeders rest and both cows die
	s->nextRound();
	s->nextRound();
	s->nextRound();
	s->nextRound();
	s->nextRound();
	s->nextRound();
	cout << "Tu dinero: " << s->getMoney() << endl;
	const Object** object_list_2;
	int tmp3_5a;
	s->getConstObjects(object_list_2, tmp3_5a);
	for (int i=0; i<tmp3_5a; i++){
		cout << "i: " << i << " " << object_list_2[i]->getName() << endl;
	}
	delete[]object_list_2;
	s->nextRound();
	...
	s->addProperty(CATTLEFARM, 0, 3);
	s->addProperty(CATTLEFARM, 0, 10);
	selected_object = s->getObjectAt(0,3);
	s->hire(dynamic_cast<Property*>(selected_object), COW);
	s->hire(dynamic_cast<Property*>(selected_object), FEEDER);


	Object *selobj2 = s->getObjectAt(0,10);
	//s->hire(dynamic_cast<Property*>(selobj2), FEEDER);

	s->nextRound();
	cout << "Round 1: " << s->getMoney() << endl; // 5
	s->nextRound();
	cout << "Round 2: " << s->getMoney() << endl; // 10
	s->nextRound();
	cout << "Round 3: " << s->getMoney() << endl; // 15
	s->nextRound();

	s->hire(dynamic_cast<Property*>(selobj2), COW);
	cout << "Round 4: " << s->getMoney() << endl; // 5
	s->nextRound();
	cout << "Round 5: " << s->getMoney() << endl; // 10
	s->nextRound();
	cout << "Round 6: " << s->getMoney() << endl; // 15


	s->getConstObjects(object_list_2, tmp3_5a);
	for (int i=0; i<tmp3_5a; i++){
		cout << "i: " << i << " " << object_list_2[i]->getName() << object_list_2[i] << endl;
	}

	s->nextRound();
	cout << "Round 7: " << s->getMoney() << endl; // 10 cow on cattlefarm 1 dies

	const Employee ** e;
	dynamic_cast<Property *>(selected_object)->getConstEmployeeList(e);
	for (int i=0; i<dynamic_cast<Property *>(selected_object)->getNumEmployee(); i++){
		cout <<"i: " << i << " " << e[i]->getName() << e[i] << endl;
	}
	delete []e;
	dynamic_cast<Property *>(selobj2)->getConstEmployeeList(e);
	for (int i=0; i<dynamic_cast<Property *>(selobj2)->getNumEmployee(); i++){
		cout <<"j: " << i << " " << e[i]->getName() << e[i] << endl;
	}
	delete []e;
